...
//}
//
//Magazin citireArticolDinFisier(FILE* f) {
//	Magazin m;
//	
//	char buffer[100];
//	char delimiter[3] = ",\n";
...
//	strcpy_s(m.denumire, strlen(token) + 1, token);
//
//	token = strtok(NULL, delimiter);
//	m.nrProduse = atoi(token);
//
//	m.preturi = malloc(sizeof(float) * m.nrProduse);
//	for (int i = 0; i < m.nrProduse; i++) {
//		token = strtok(NULL, delimiter);
//		m.preturi[i] = atof(token);
//	}
//
//	return m;
//}
//
//void adaugaArticolInVector(Magazin** vector, int* dim, Magazin nou) {
//	Magazin* temp = malloc(sizeof(Magazin) * ((*dim) + 1));
//	for (int i = 0; i < *dim; i++) {
//		temp[i] = (*vector)[i];
//	}
//
...
//
//
//Magazin* citireVectorDinFisier(const char* numeFisier, int* nrElemCitite) {
//	Magazin* vector = NULL;
...
//	else {
//		while (!feof(f)) {
//			Magazin m = citireArticolDinFisier(f);
//			adaugaArticolInVector(&vector, nrElemCitite, m);
//		}
//
//		fclose(f);
//		return vector;
...
//}
//
//void adaugareArticolInFisier(FILE* f, Magazin m) {
//	if (!f) {
//		printf("Fisierul nu a putut fi deschis!\n");
//	}
//	else {
//		fprintf(f, "%d,%s,%d", m.cod, m.denumire, m.nrProduse);
//		for (int i = 0; i < m.nrProduse; i++) {
//			fprintf(f, ",%.2f", m.preturi[i]);
//		}
//
//		fprintf(f, "\n");
//	}
//}
//
//void adaugareVectorInFisier(FILE* f, Magazin* vector, int dim) {
//	if (!f) {
//		printf("Fisierul nu a putut fi deschis!\n");
//	}
//	else {
//		for (int i = 0; i < dim; i++) {
//			adaugareArticolInFisier(f, vector[i]);
//		}
//	}
//}
//
...
//
//	//Magazin articolCitit = citireArticolDinFisier(f); 
//	//afisareArticol(articolCitit);
//	//printf("\n\n"); 
//
//	//fclose(f);
//
//	int dimensiune;
//	Magazin* vCitit = citireVectorDinFisier("Text.txt", &dimensiune);
//	//afisareVector(vCitit, dimensiune);
//
//	FILE* f2 = fopen("Text2.txt", "w");
//	adaugareVectorInFisier(f2, vector, dim);
//	fclose(f2);
//
//	int dimtest = 5;
//	dezalocare(&vector, &dimtest); 
//	dezalocare(&medieMare, &dim); 
//	dezalocare(&vCitit, &dimensiune);
